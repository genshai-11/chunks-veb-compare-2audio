...
| --- | --- | --- | --- |
| CAND-001 | Keep detected stack as-is (static vanilla ES modules, no build, vitest, Vercel) | fully working app, requirements met | low |
| CAND-002 | Accept reconstructed build plan (verify F01–F04 → cleanup F05 → release F06) | git history matches plan doc | low |
| CAND-003 | Repo cleanup: move/delete `src/.pi/gsd`, delete server logs, merge DESIGN.md into 07-ui-system, add `.gitattributes` + renormalize line endings | hygiene audit | medium (line-ending commit touches 8 files) |
